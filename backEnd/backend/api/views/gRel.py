    ...
    with transaction.atomic():
        # try:
            for conceleItem in relEleLstItem:
                ...
                if item:    
                    item.lstRel.append(conceleItem.relation.lower() if conceleItem.relation else "")
                    item.lstidlaw.append(conceleItem.IdLaw.id)
                    item.meaning if item.meaning else conceleItem.Meaning
                    item.descendants.extend(conceleItem.descendants)
                    ...
                    kwargs = {
                        ...
                        'lstRel': [conceleItem.relation.lower() if conceleItem.relation else ""],
    ...
    try:
        ...
        for item in queryset:
            results.append({
                ...
                "lstid": item.lstid,
                "lstRel": list(dict.fromkeys(item.lstRel)),
...
@permission_classes([IsAdmin])  
@api_view(['POST'])
def create_grel(request):
    serializer = gRelSerializer(data=request.data)
